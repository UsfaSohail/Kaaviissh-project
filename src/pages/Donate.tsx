    ...
    return (
      <div className="pt-24 pb-16 px-4 min-h-screen flex items-center justify-center">
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center max-w-md">
          ...
          <p className="text-muted-foreground italic">"May your charity bring ease to those in need."</p>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="pt-24 pb-16 px-4 min-h-screen">
      <div className="container mx-auto max-w-lg">
        <h1 className="text-3xl font-bold text-foreground text-center mb-2">Donate</h1>
        <p className="text-center text-muted-foreground mb-10">Your generosity can change lives.</p>
          ...
          >
            {step === 0 && (
              <div className="space-y-3">
                <h2 className="text-lg font-semibold text-foreground mb-4">Choose Donation Type</h2>
                {donationTypes.map((t) => (
                  <button
                    key={t}
                    onClick={() => setType(t)}
            ...
            {step === 1 && (
              <div>
                <h2 className="text-lg font-semibold text-foreground mb-4">Enter Amount (PKR)</h2>
                <input
                  type="number"
                  value={amount}
                  onChange={(e) => setAmount(e.target.value)}
                  placeholder="e.g. 5000"
                  className="w-full px-5 py-4 rounded-xl bg-card border border-border text-foreground text-lg placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"
                />
            ...
            {step === 3 && method && (
              <div className="bg-card border border-border rounded-2xl p-6 text-center space-y-4">
                ...
                <p className="text-xs text-muted-foreground">After sending, proceed to upload your payment screenshot.</p>
              </div>
            )}

            {step === 4 && (
              <div className="text-center space-y-6">
                <h2 className="text-lg font-semibold text-foreground">Upload Payment Screenshot</h2>
