            ...
            return (
              <button key={uid} onClick={() => handleSelectUser(uid)} className={`w-full text-start p-4 border-b border-border/50 hover:bg-secondary/30 transition-colors ${selectedUser === uid ? "bg-secondary/50" : ""}`}>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-foreground font-medium truncate">{userNames[uid] || `User ${uid.slice(0, 6)}`}</span>
          ...
          {selectedUser ? (
            <>
              <div className="p-3 border-b border-border flex items-center justify-between">
                <span className="text-sm text-foreground font-medium">Chat with {userNames[selectedUser] || `User ${selectedUser.slice(0, 6)}`}</span>
              </div>
